            ...
            <div className={className} data-testid={`progress-slot-${slot.id}`}>
                <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="text-right">
                        <div className="text-3xl font-black text-[var(--ember)]">{Math.round(progress)}%</div>
                        <div className="text-sm text-[var(--forge-text-muted)]">Complete</div>
                    </div>
                </div>

                {/* Progress Bar */}
                <div className="h-3 bg-[var(--forge-bg-elevated)] rounded-full overflow-hidden mb-4">
                    <motion.div
                        initial={{ width: 0 }}
                ...
                <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-4 text-[var(--forge-text-muted)]">
                    ...
                    <button
                        className={cn(
                            ...
                            "text-[var(--ember)] font-medium hover:underline"
                        )}
    ...
    if (variant === "inline") {
        return (
            <div className={`flex items-center gap-4 ${className ?? ""}`} data-testid={`progress-slot-${slot.id}`}>
                ...
                <span className="text-sm font-medium text-[var(--forge-text-secondary)]">{Math.round(progress)}%</span>
            </div>
        );
    }
                            ...
                            {simplifiedSections.map((section, i) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        currentSection === i
                                            ? "bg-[var(--ember)]/10 border-2 border-[var(--ember)]/30"
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-xs text-[var(--forge-text-muted)]">{section.time}</div>
                                    </div>
                                </button>
                            ))}
                        </div>
