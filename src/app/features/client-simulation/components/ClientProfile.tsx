            ...
            <div className="p-4">
                ...
                <div className="grid grid-cols-3 gap-2 mt-4">
                    ...
                    <QuickStat
                        ...
                        color={persona.patience === "low" ? "red" : persona.patience === "high" ? "emerald" : "amber"}
                    />
                    <QuickStat
                        icon={<DollarSign size={ICON_SIZES.sm} />}
                        label="Budget"
                        value={persona.budgetSensitivity}
                        color={persona.budgetSensitivity === "strict" ? "red" : persona.budgetSensitivity === "flexible" ? "emerald" : "amber"}
                    />
                </div>
            </div>
            ...
            {showDetails && (
                <>
                    <button
                        ...
                        className="w-full px-4 py-2 flex items-center justify-between text-sm text-[var(--text-secondary)] hover:bg-[var(--surface-overlay)] transition-colors border-t border-[var(--border-subtle)]"
                    >
...
const StyleTag: React.FC<StyleTagProps> = ({ label, value }) => {
    return (
        <div className="px-2 py-1 rounded-lg bg-[var(--surface-overlay)]">
            <span className="text-xs text-[var(--text-muted)]">{label}: </span>
            <span className="text-xs font-medium text-[var(--text-secondary)] capitalize">{value}</span>
        </div>
    );
};
