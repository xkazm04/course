...
# LayoutTemplate as Pure Data, ChapterView as Interpreter
...
- **Generated**: 12/23/2025, 10:14:20 PM

## Description
The LayoutTemplate is currently a static configuration object, but ChapterView contains imperative rendering logic in modeTemplates. Invert this: make LayoutTemplate include all mode-specific metadata (wrapperClass, dataTestId, responsive config) so ChapterView becomes a pure interpreter with zero conditional logic. Each mode becomes a complete declarative specification rather than code.

## Reasoning
This eliminates the modeTemplates Record inside ChapterView and the conditional wrapperClass logic. Adding a new mode becomes pure data addition with no code changes. The template becomes the single source of truth, making the system more predictable and testable.
