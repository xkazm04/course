# Single Feature Context Scan: skill-progress

## Your Mission

Analyze the **skill-progress** feature folder and create intelligent, well-structured contexts for this feature. Each context should represent a cohesive sub-feature or module within this feature. **Additionally, you must match each context to an existing context group based on the group names.**
...
- **Project Type**: nextjs
- **Feature Name**: skill-progress
- **Feature Path**: src/app/features/skill-progress
...
find "src/app/features/skill-progress" -type f -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" | head -n 50
```
...
curl -X POST "http://localhost:3000/api/file-dependencies" \
  ...
  -d '{"filePath":"C:/Users/kazda/kiro/vibeman/src/app/features/skill-progress/MainFile.tsx","projectPath":"C:/Users/kazda/kiro/vibeman","maxDepth":3}'
```
...
**Naming Convention:**
- Use feature name + specific area: "skill-progress - [Sub-feature/Module]"
- Examples:
...
curl -X POST "http://localhost:3000/api/contexts" \
  -H "Content-Type: application/json" \
  -d '{"projectId":"abf3fcda-5390-4502-90b8-3e8cb5312342","groupId":"<matched-group-id-from-step-0>","name":"skill-progress - Context Name","description":"## Overview\n...","filePaths":["src/app/features/skill-progress/file1.tsx","src/app/features/skill-progress/file2.ts"],"testScenario":"[{\"type\":\"navigate\",\"url\":\"http://localhost:3001/route\"},{\"type\":\"wait\",\"delay\":3000}]"}'
```
...
- `name`: "skill-progress - [Specific area]"
...
- [ ] Context names include feature name: "skill-progress - [Area]"
...
**Begin analyzing the skill-progress feature now. Focus on creating high-quality, well-structured contexts that accurately represent the feature's architecture.**
