...
# Delete unused ThemeCompiler re-exports from landing index
...
- **Generated**: 12/23/2025, 7:24:11 PM

## Description
The landing/index.ts re-exports 10 ThemeCompiler symbols (themeCompiler, ThemeCompiler, ThemeIntent, ThemePreset, getPreset, listPresets, getPresetsByCategory, useCompiledTheme, useThemePresets, useStaticTheme, useAccessibleTheme) that are already available from @/app/shared/lib/variantMachine. A grep shows zero imports of these from the landing feature. Remove these re-exports to eliminate the re-export indirection and simplify the public API.

## Reasoning
Re-exporting symbols that consumers can already import from the canonical source adds no value and creates multiple import paths for the same symbols. Clean public API means less confusion for developers.
