# Single Feature Context Scan: user-learning-graph

## Your Mission

Analyze the **user-learning-graph** feature folder and create intelligent, well-structured contexts for this feature. Each context should represent a cohesive sub-feature or module within this feature. **Additionally, you must match each context to an existing context group based on the group names.**

## Feature Information

- **Project ID**: abf3fcda-5390-4502-90b8-3e8cb5312342
- **Project Name**: course
- **Project Path**: C:\Users\kazim\dac\course
- **Project Port**: 3001 (use `http://localhost:3001` for test scenarios)
- **Project Type**: nextjs
- **Feature Name**: user-learning-graph
- **Feature Path**: src/app/features/user-learning-graph
...
```bash
curl -X GET "http://localhost:3000/api/context-groups?projectId=abf3fcda-5390-4502-90b8-3e8cb5312342"
```
...
find "src/app/features/user-learning-graph" -type f -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" | head -n 50
```
...
curl -X POST "http://localhost:3000/api/file-dependencies" \
  ...
  -d '{"filePath":"C:/Users/kazda/kiro/vibeman/src/app/features/user-learning-graph/MainFile.tsx","projectPath":"C:/Users/kazda/kiro/vibeman","maxDepth":3}'
```
...
**Naming Convention:**
- Use feature name + specific area: "user-learning-graph - [Sub-feature/Module]"
- Examples:
...
1. **Start from homepage**: `http://localhost:3001`
...
**Valid step types:**
- `navigate`: `{ "type": "navigate", "url": "http://localhost:3001/path" }`
...
curl -X POST "http://localhost:3000/api/contexts" \
  -H "Content-Type: application/json" \
  -d '{"projectId":"abf3fcda-5390-4502-90b8-3e8cb5312342","groupId":"<matched-group-id-from-step-0>","name":"user-learning-graph - Context Name","description":"## Overview\n...","filePaths":["src/app/features/user-learning-graph/file1.tsx","src/app/features/user-learning-graph/file2.ts"],"testScenario":"[{\"type\":\"navigate\",\"url\":\"http://localhost:3001/route\"},{\"type\":\"wait\",\"delay\":3000}]"}'
```
...
- `name`: "user-learning-graph - [Specific area]"
...
```bash
curl -X GET "http://localhost:3000/api/contexts?projectId=abf3fcda-5390-4502-90b8-3e8cb5312342"
```
...
- [ ] Test scenarios start from `http://localhost:3001`
- [ ] Test scenarios use data-testid selectors
- [ ] Context names include feature name: "user-learning-graph - [Area]"
...
**Begin analyzing the user-learning-graph feature now. Focus on creating high-quality, well-structured contexts that accurately represent the feature's architecture.**
